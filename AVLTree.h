...
#ifndef DESIGNERMODE_AVLTREE_H
#define DESIGNERMODE_AVLTREE_H
...
public:
    ...
    AVLTreeNode *right;    // 右孩
    AVLTreeNode(T key, T value, AVLTreeNode *l, AVLTreeNode *r) : key(key),
                                                                  value(value), height(0), left(l), right(r) {}
};
...
AVLTreeNode<T> *AVLTree<T>::search(AVLTreeNode<T> *x, T key) const {
    if (x == NULL || x->key == key) //这里的value是为了防止key相同的情况
        return x;
